...
<script>
    ...
    $(document).ready(function() {
        ...
        console.log('log')
        $.get("http://localhost:8000/csm/companycount", function(data, status){
            $.each(data.data.companies, function(index, value) {
              $("#companies").text(value.total +' Data')
            });
        });

        $.get("http://localhost:8000/csm/outletcount", function(data, status){
            $.each(data.data.outlets, function(index, value) {
              $("#outlets").text(value.total +' Data')
            });
        });

        $.get("http://localhost:8000/csm/devicecount", function(data, status){
            $.each(data.data.devices, function(index, value) {
              $("#devices").text(value.total +' Data')
            });
        });

        $.get("http://localhost:8000/csm/transactionscount", function(data, status){
