...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#nameuser").text(nama);
        $.get("http://localhost:8000/cse/companycount/" + user_id, function(data, status){
            //alert(data.data.companies);
            $.each(data.data.companies, function(index, value) {
              $("#companies").text(value.total +' Data')
            });
        });

        $.get("http://localhost:8000/cse/outletcount/" + user_id, function(data, status){
            //alert(data.data.outlets);
            $.each(data.data.outlets, function(index, value) {
              $("#outlets").text(value.total +' Data')
            });
        });

        $.get("http://localhost:8000/cse/devicecount/" + user_id, function(data, status){
            $.each(data.data.devices, function(index, value) {
              $("#devices").text(value.total +' Data')
            });
        });

        $.get("http://localhost:8000/cse/transactionscount/" + user_id, function(data, status){
            //alert(data.data.outlets);
