...
<script>
    ...
    $.get("http://localhost:8000/listtransactions",  function(data, status){
        $.each(data.data.transactions, function(index, value) {
            var body = '<tr>'
              body += '<td>'+value.receipt_code+'</td>'
              body += '<td>'+value.customer_name+'</td>'
              body += '<td>'+value.final_amount+'</td>'
              body += '</tr>';
              $("#transactions tbody").append(body);
        });
